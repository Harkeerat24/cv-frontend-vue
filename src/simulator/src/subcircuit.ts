...
export default class SubCircuit extends CircuitElement {
  ...
  constructor(
    ...
    if (subcircuitScope == undefined || subcircuitScope.checkDependency(scope.id)) {
      if (savedData) {
  ...
  reset(): void {
    ...
    for (const id in temp_map_inp) {
      if (temp_map_inp[id][1]) {
        if (
          temp_map_inp[id][0].layoutProperties.x == temp_map_inp[id][1].x &&
          temp_map_inp[id][0].layoutProperties.y == temp_map_inp[id][1].y
        ) {
    ...
    this.inputNodes = [];
    for (let i = 0; i < subcircuitScope.Input.length; i++) {
      const input = temp_map_inp[subcircuitScope.Input[i].layoutProperties.id][0];
    ...
    for (const id in temp_map_out) {
      if (temp_map_out[id][1]) {
        if (
          temp_map_out[id][0].layoutProperties.x == temp_map_out[id][1].x &&
          temp_map_out[id][0].layoutProperties.y == temp_map_out[id][1].y
        ) {
    ...
    this.outputNodes = [];
    for (let i = 0; i < subcircuitScope.Output.length; i++) {
      const output = temp_map_out[subcircuitScope.Output[i].layoutProperties.id][0];
  ...
  customDraw(): void {
    ...
    for (let i = 0; i < subcircuitScope.Input.length; i++) {
      ...
      const info = this.determine_label(this.inputNodes[i].x, this.inputNodes[i].y);
    ...
    for (let i = 0; i < subcircuitScope.Output.length; i++) {
      ...
      const info = this.determine_label(this.outputNodes[i].x, this.outputNodes[i].y);
      ctx.textAlign = info[0];
      fillText(
        ctx,
